...
class Bar(object):
	...
	def calculate(self,client,slope): #コントローラーの傾きのデータ(-1～1)からバーの位置と速度を計算する
		val=1.0*min(self.calib[1],max(self.calib[0],slope))#self.calibの範囲内に
		movable=[self.region[0]+self.width/2.0,self.region[1]-self.width/2.0]#バーの中心の移動可能域
		pos=movable[0]+(movable[1]-movable[0])*(val-self.calib[0])/float(self.calib[1]-self.calib[0])#重心位置を移動可能域と線形に対応付け
		self.position = pos
		client.sendData('bar_position',['Client','Game'],[pos,self.width])